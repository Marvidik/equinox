...
export default function ReferralPage() {
  ...
  const filtered = referrals.filter(
    r => (r.client_name || '').toLowerCase().includes(search.toLowerCase()) ||
         (r.client_status || '').toLowerCase().includes(search.toLowerCase())
  );
  ...
  const activeCount   = referrals.filter(r => r.client_status === 'active').length;
    ...
    <div className={styles.page}>
      ...
      <div className={styles.shareCard}>
        ...
        <div className={styles.socialRow}>
          ...
          <a
            href={`https://twitter.com/intent/tweet?text=Join+Equinox+Global+Assets+and+start+investing!+Use+my+referral+link:+${encodeURIComponent(referralLink)}`}
          ...
          <a
            href={`https://wa.me/?text=Join+Equinox+Global+Assets!+${encodeURIComponent(referralLink)}`}
          ...
          <a
            href={`mailto:?subject=Join Equinox Global Assets&body=Join me on Equinox Global Assets! Use my referral link: ${referralLink}`}
        ...
        <div className={styles.tableWrap}>
          <table className={styles.table}>
            ...
            <tbody>
              {loading ? (
                <tr>
                  <td colSpan={4} className={styles.emptyCell}>Loading referrals...</td>
                </tr>
              ) : sliced.length === 0 ? (
                <tr>
                ...
                <tr key={r.id || i}>
                  <td className={styles.nameCell}>
                    <div className={styles.nameAvatar}>{(r.client_name || 'U').charAt(0).toUpperCase()}</div>
                    {r.client_name}
                  </td>
                  <td>
                    <span className={styles.levelBadge}>Level {r.ref_level || 1}</span>
                  </td>
                  <td>
                    <span className={`${styles.statusBadge} ${r.client_status === 'active' ? styles.status_active : styles.status_pending}`}>
                      {(r.client_status || 'pending').charAt(0).toUpperCase() + (r.client_status || 'pending').slice(1)}
                    </span>
                  </td>
                  <td className={styles.dateCell}>{r.date_registered ? new Date(r.date_registered).toLocaleString() : 'N/A'}</td>
                </tr>
              ))}
            </tbody>
          </table>
